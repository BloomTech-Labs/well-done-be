...
const url =
	// "https://wyz1igf1ui.execute-api.us-west-2.amazonaws.com/dev/getMomoStatus?id=";
...
async function getData(url, entryNum) {
	let sensors = await db("sensors").select("physical_id");
	// .where("id", "=", 1);

	for await (const [idx, sensor] of sensors.entries()) {
		const resMomo = await axios.get(
			`${url}${sensor.physical_id}&nDays=${entryNum}`
		);

		if (resMomo.data.dates.length === 0) {
			console.log(sensor.physical_id);
			// let today = new Date();
			...
			// 		count: null,
			// 		total: null,
			// 		sensor_id: sensor.physical_id,
			// 		reported_percent: null,
			// 		created_at: today
			// 	};

			// 	let id = await db("history")
			// 		.insert(dayData)
			// 		.returning("id");
			...
			// 	await db("pad_counts").insert(padCounts);
			// 	await db("pad_seconds").insert(padSeconds);
			// }
		} else {
			let today = new Date();
			let dd = String(today.getDate()).padStart(2, "0");
			...
			for await (const [
				index,
				date
			] of resMomo.data.dates.reverse().entries()) {
				dd = dd - counter;
				if (dd < 1) {
					mm--;
					if (mm === 5) {
				...
				let dayData = {
					date: date,
					count: resMomo.data.statuses[entryNum - 1 - index].count,
					total: resMomo.data.statuses[entryNum - 1 - index].total,
					sensor_id: sensor.physical_id,
					status: resMomo.data.statuses[entryNum - 1 - index].status,
					reported_percent:
						resMomo.data.statuses[entryNum - 1 - index].reportedPercent,
					created_at: today
				};
				...
				let padSeconds = {
					pad_seconds_0:
						resMomo.data.statuses[entryNum - 1 - index].padSeconds[0],
					pad_seconds_1:
						resMomo.data.statuses[entryNum - 1 - index].padSeconds[1],
					pad_seconds_2:
						resMomo.data.statuses[entryNum - 1 - index].padSeconds[2],
					pad_seconds_3:
						resMomo.data.statuses[entryNum - 1 - index].padSeconds[3],
					history_id: id[0]
				};

				let padCounts = {
					pad_count_0: resMomo.data.statuses[entryNum - 1 - index].padCounts[0],
					pad_count_1: resMomo.data.statuses[entryNum - 1 - index].padCounts[1],
					pad_count_2: resMomo.data.statuses[entryNum - 1 - index].padCounts[2],
					pad_count_3: resMomo.data.statuses[entryNum - 1 - index].padCounts[3],
					history_id: id[0]
				};
		...
		console.log(`Complete! ${sensor.physical_id}`);
	}
}
